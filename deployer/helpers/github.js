...
function getGithubAuthHeaders() {
  return {
    ...
    Authorization: `Basic ${Buffer.from(
      GITHUB_USER_AGENT + ":" + GITHUB_PERSONAL_ACCESS_TOKEN
    ).toString("base64")}`,
  };
}
...
async function getPackageDetailsForOrg(owner) {
  ...
  const r = await octokit.request(
    "GET https://api.github.com/orgs/{owner}/packages",
    {
      owner,
...
async function getPackageVersions(owner, name, offset = 1, limit = 100) {
  ...
  const r = await octokit.request(
    "GET https://api.github.com/orgs/{owner}/packages/container/{name}/versions",
    {
      name,
      owner,
      per_page: limit,
...
async function getPackages(owner, name, maxPackages = 100) {
  let result = [];
  let pageInfo = { nextPage: 1 };
  while (pageInfo.nextPage && result.length <= maxPackages) {
    const offset = pageInfo.nextPage;
    const [returnedPageInfo, items] = await getPackageVersions(
      owner,
      name,
      offset
    );
...
async function getPullRequestDetails(url, headers) {
  ...
  await Promise.all(
    branches.map(async (branch) => {
      ...
      let urls = (body.match(urlRegexp) || [])
        .filter((url) => typeof url === "string" && url.indexOf("asana") !== -1)
