...
const {
  API_BRANCHES_URL,
  GITHUB_WEB_BRANCHES_URL,
  API_PULL_REQUEST_URL,
  GITHUB_GRAPHQL_PACKAGES_WEB,
  GITHUB_GRAPHQL_PACKAGES_API,
  RELEASES_DIR,
  DEBUG,
} = process.env;
...
const branches = async function (req, res) {
  try {
    ...
    const [api_owner, _api_package, ...apiPackageName] =
      GITHUB_GRAPHQL_PACKAGES_API.split("/");
    const apiDockerImages = await getPackages(
      api_owner,
      apiPackageName.join("/")
    );
    ...
    let apiReleaseBranches = apiRemoteBranches.map((branch) => {
      ...
      return {
        ...
        hasDockerImage: !!apiDockerImages.find(
          (d) => cleanBranch(d.name) === localBranch
        ),
        ports: {},
        createdAt,
...
const executeInstanceScript = async function (
  localBranchParam,
  script,
  params = [],
  returnResult = false
  ...
  console.log(
    `Executing: "${script} ${params.join(" ")}" on ${localBranchParam}`
  );
  if (!isValidLocalBranch(localBranchParam)) {
    return 401;
  }
  const result = await exec(
    `cd ${RELEASES_DIR}/${localBranch} && ./${script} ${params.join(" ")}`
  );
...
const linkToWeb = async function (req, res) {
  ...
  return res.sendStatus(
    await executeInstanceScript(req.params.localBranch, "manage-instance.sh", [
      "--web",
      `'${webBranch}'`,
      "&",
    ])
...
const deleteScratch = async function (req, res) {
  return res.sendStatus(
    await executeInstanceScript(req.params.localBranch, "delete.sh")
  );
};

const extractEnvValues = async function (localBranch) {
  const { stdout: env, stderr } = await executeInstanceScript(
    localBranch,
    "manage-instance.sh",
    ["--env"],
    true
...
const resetEnv = async function (req, res) {
  ...
  const envFileNames = Object.keys(currentEnvs).map((f) =>
    f.replace("env.d/", "")
  );
  if (envs.length === 0) {
    envs = envFileNames;
